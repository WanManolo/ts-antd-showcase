...
const documents: Document[] = [
  {
    ...
    status: "Printed",
    address: "Address 1",
    date: moment(new Date("01/31/2021"), dateFormat)
      .format(dateFormat)
      .toLocaleString(),
    channel: "PDF",
  ...
  },
  {
    ...
    status: "Printed",
    address: "Address 12",
    date: moment(new Date("01/01/2021"), dateFormat)
      .format(dateFormat)
      .toLocaleString(),
    channel: "SMS",
  ...
  {
    key: 2,
    name: "Name Document 3",
    group: "Group 3",
    type: "type 1",
    status: "Sorted",
    address: "Address 123",
    date: moment(new Date("09/28/2021"), dateFormat)
      .format(dateFormat)
      .toLocaleString(),
    channel: "PDF",
    content: "This is the content of the document 3",
    action: null,
  },
  {
    key: 3,
    name: "Name Document 4",
    group: "Group 4",
    type: "type 3",
    status: "Received",
    address: "Address 1234",
    date: moment(new Date("10/11/2021"), dateFormat)
      .format(dateFormat)
      .toLocaleString(),
    channel: "SMS",
    content: "This is the content of the document 4",
    action: null,
  },
];
...
export const DocumentTable: FC<Props> = ({
  ...
  const handleAdd = () => {
    const newData: Document = {
      ...
      name: form.getFieldValue("name")
        ...
        : `New Document ${documentList.length + 1}`,
      group: "Group 5",
      type: "type 3",
      address: "Address 1234",
      status: "Sorted",
    ...
    return (
      <Row>
        ...
        <Col span={2} offset={17}>
          <Select
            defaultValue="date"
            onChange={(value: string) => handleResetColumnChange(value)}
          >
        ...
        <Col span={2}>
          <Button onClick={() => resetFilters(selectedColumn)}>Reset</Button>
        </Col>
      </Row>
    );
  };
  ...
  useEffect(() => {
    const newList = documentList
      ...
      .filter(
        (doc: Document) =>
          addressFilter === "" || doc.address.includes(addressFilter)
      )
      .filter(
        (doc: Document) =>
          statusFilter.length === 0 ||
          statusFilter.some((sf) => doc.status === sf)
      )
      .filter(
        (doc: Document) => groupFilter === "" || doc.group.includes(groupFilter)
      )
      .filter((doc: Document) => {
        return (
          ...
          dateRangeFilter[0] === "" ||
          dateRangeFilter[1] === "" ||
          (moment(dateRangeFilter[0], dateFormat).unix() <=
            moment(doc.date, dateFormat).unix() &&
            moment(dateRangeFilter[1], dateFormat).unix() >=
              moment(doc.date, dateFormat).unix())
        );
      });
